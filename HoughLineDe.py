...
# 차선 인식에 가장 많은 어려움이 있는 프레임을 'frame1'폴더에 따로 저장하기 (Debug)
import os
...
if VIDEO == True:
   ...
   print(edgeList)
   src = cv2.imread(os.path.join(edgeImageDir, edgeList[0]))
   frame_size = src.shape[1], src.shape[0]
   counter = 0
...
while(True):
   if VIDEO == True:
      ...
      if counter>len(edgeList):
         break

      src = cv2.imread(os.path.join(edgeImageDir, edgeList[counter]))
      counter +=1
   ...
   vertices = np.array(
      [[(133, height-70), (width/2-15, height/2+120), (width/2+10, height/2+120), (width-133, height-70)]],
      dtype=np.int32)
